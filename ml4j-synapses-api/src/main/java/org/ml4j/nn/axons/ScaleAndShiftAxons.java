 ...
 *
 */
public interface ScaleAndShiftAxons
    extends TrainableAxons<Neurons, Neurons, ScaleAndShiftAxons> {

  /**
   * 
   * @return The row vector containing the scaling weights - one for each non-bias Neuron.
   */
  Matrix getScaleRowVector();

  /**
   * 
   * @return The row vector containing the shift weights - one for each non-bias Neuron.
   */
  Matrix getShiftRowVector();
}
